...
@push('styles')
    ...
    <style>
        ...
        /* Stats Grid */
        .stats-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 1rem; max-width: 400px; margin: 0 auto; }
...
<div class="min-h-screen page-bg">
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 scroll-target">
        ...
        <div class="mb-12 fade-in-up">
            <form action="{{ route('artikel.public.index') }}" method="GET" class="flex justify-center mb-6">
            ...
            <div class="flex flex-wrap justify-center gap-3 filter-container">
                <a href="{{ route('artikel.public.index', ['search' => request('search')]) }}"
                   class="filter-btn px-5 py-2.5 rounded-xl font-semibold {{ !request('kategori') ? 'active-filter' : 'text-teal-700 bg-white/80 border border-teal-200 shadow-sm hover:shadow-md' }}">
                    <i class="fas fa-th-large mr-2"></i>Semua Artikel
                </a>
                @foreach ($kategori as $kat)
                <a href="{{ route('artikel.public.index', ['kategori' => $kat->nama_kategori, 'search' => request('search')]) }}"
                   class="filter-btn px-5 py-2.5 rounded-xl font-semibold {{ request('kategori') == $kat->nama_kategori ? 'active-filter' : 'text-teal-700 bg-white/80 border border-teal-200 shadow-sm hover:shadow-md' }}">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        images.forEach(img => {
            ...
            const showImg = () => { img.classList.remove('opacity-0'); img.classList.add('opacity-100'); };
